...
export function processRecipeRequest({ method, pathname, body = {}, data }) {
  ...
  if (method === 'POST' && pathname === '/api/data' && body.action === 'bulk-delete') {
    const type = body.type
    const ids = [...new Set(Array.isArray(body.ids) ? body.ids.filter((id) => typeof id === 'string' && id) : [])]
    if (!['recipe', 'preparation', 'ingredient'].includes(type) || ids.length === 0) {
      return result(400, { error: 'Choose at least one valid item to delete' }, data)
    }

    const idSet = new Set(ids)
    if (type === 'preparation') {
      const blockedIds = ids.filter((id) =>
        data.recipes.some((recipe) => recipe.preparations?.some((item) => item.preparationId === id)),
      )
      if (blockedIds.length) {
        return result(409, {
          error: 'Some selected preparations are used in drinks and cannot be deleted',
          code: 'ITEMS_IN_USE',
          blockedIds,
        }, data)
      }
    ...
    if (type === 'ingredient') {
      const blockedIds = ids.filter((id) =>
        data.recipes.some((recipe) => recipe.ingredients.some((item) => item.ingredientId === id))
        || data.preparations.some((preparation) => preparation.ingredients.some((item) => item.ingredientId === id)),
      )
      if (blockedIds.length) {
        return result(409, {
          error: 'Some selected ingredients are in use and cannot be deleted',
          code: 'ITEMS_IN_USE',
          blockedIds,
        }, data)
      }
    ...
    const before = data[collectionKey].length
    const deletedIds = data[collectionKey].filter((item) => idSet.has(item.id)).map((item) => item.id)
    data[collectionKey] = data[collectionKey].filter((item) => !idSet.has(item.id))
    return result(200, { ok: true, deletedIds }, data, data[collectionKey].length !== before)
  }
  ...
  if (ingredientMatch && method === 'DELETE') {
    ...
    if (data.ingredients.length === before) return result(200, { ok: true, alreadyDeleted: true }, data)
  ...
  if (preparationMatch && method === 'DELETE') {
    ...
    if (data.preparations.length === before) return result(200, { ok: true, alreadyDeleted: true }, data)
    return result(200, { ok: true }, data, true)
  }
  ...
  if (recipeMatch && method === 'DELETE') {
    ...
    if (data.recipes.length === before) return result(200, { ok: true, alreadyDeleted: true }, data)
    return result(200, { ok: true }, data, true)
  }
